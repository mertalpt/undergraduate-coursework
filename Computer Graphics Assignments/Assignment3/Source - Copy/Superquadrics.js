...
const MATERIAL_DIFFUSE = vec4(0.8, 0.6, 0.2, 1.0);
const MATERIAL_SPECULAR = vec4(0.4, 0.4, 0.6, 1.0);
...
function ellipsoid_surface(eta, omega)
{
    ...
    let cached = Math.pow(Math.cos(eta), EPSILON_1);

    return vec4(
        ALPHA_1 * cached * Math.pow(Math.cos(omega), EPSILON_2),
        ALPHA_2 * cached * Math.pow(Math.sin(omega), EPSILON_2),
...
{
    // Cache for efficiency
    let cached = Math.pow(Math.cos(eta), 2 - EPSILON_1);

    return vec4(
        (1 / ALPHA_1) * cached * Math.pow(Math.cos(omega), 2 - EPSILON_2),
        (1 / ALPHA_2) * cached * Math.pow(Math.sin(omega), 2 - EPSILON_2),
        (1 / ALPHA_3) * Math.pow(Math.sin(eta), 2 - EPSILON_1),
        0.0
    );
}
...
function toroid_surface(eta, omega)
{
    ...
    let cached = ALPHA_4 + Math.pow(Math.cos(eta), EPSILON_1);
